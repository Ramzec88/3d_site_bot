...
export default function Page() {
  ...
  const bots = [
    { id: 1, title: "Tarot Whisper | Шёпот карт", desc: "Карта дня и расклады.", votes: 128 },
    { id: 2, title: "SongGift", desc: "Персональные песни по заявке.", votes: 312 },
    { id: 3, title: "TravelDeal Hunter", desc: "Авиабилеты и отели.", votes: 98 },
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#0b1220] to-[#0b1220] text-white">
      ...
      <header className="mx-auto max-w-7xl px-4 pt-10 pb-8 grid md:grid-cols-2 gap-8 items-center">
        <div className="space-y-5">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold leading-tight">Найди идеального бота для любых задач</h1>
          <p className="text-white/70 max-w-prose">Честные обзоры, живая лента отзывов и удобные фильтры.</p>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <a className="inline-flex items-center justify-center h-11 px-5 rounded-2xl bg-cyan-500 text-black font-medium" href="#list">
              Добавить обзор
            </a>
          </div>
        </div>
      ...
      <main id="list" className="mx-auto max-w-7xl px-4 pb-16 grid md:grid-cols-2 xl:grid-cols-3 gap-4">
        ...
        {!bots.length && <div className="col-span-full text-center text-white/60 py-12">Ничего не найдено.</div>}
      </main>

      <footer className="border-t border-white/10">
        <div className="mx-auto max-w-7xl px-4 py-8 text-white/60 text-sm">© {new Date().getFullYear()} BotScope</div>
      </footer>
    </div>
  );
}
